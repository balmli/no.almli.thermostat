...
const SUPPORTED_CLASSES = ['fan', 'heater', 'sensor', 'socket', 'thermostat'];
const SUPPORTED_CAPABILITIES = ['onoff', 'measure_temperature', 'measure_humidity', 'target_temperature', 'thermostat_mode', 'vt_onoff', 'vh_target_humidity'];
...
module.exports = class DeviceHandler extends EventEmitter {
        ...
        if (capabilities) {
            for (let key in capabilities) {
                if (capabilities.hasOwnProperty(key) &&
                    SUPPORTED_CAPABILITIES.includes(key)) {
    ...
    async _registerDevice(device) {
        ...
        if (!SUPPORTED_CLASSES.includes(device.class)) {
            return;
        }
        ...
        if (createNode) {
            ...
            if (device.driverId === 'VThermo' || device.driverId === 'VHumidity') {
    ...
    getTemperatures(zone, tempSettings) {
        ...
        if (zone &&
            tempSettings &&
            (tempSettings.sensor || tempSettings.thermostat || tempSettings.other)) {
            ...
            for (let z of zones) {
                temperatures.push(...this.getDevicesInZone(z)
                    .filter(d => tempSettings.sensor && d.class === 'sensor' ||
                        tempSettings.thermostat && d.class === 'thermostat' && d.driverId !== 'VThermo' ||
            ...
            //this.log(`getTemperatures:`, zone, tempSettings, temperatures);
        }
        return temperatures;
    }
