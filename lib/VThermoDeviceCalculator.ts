...
export class VThermoDeviceCalculator extends DeviceCalculator {
    ...
    private validateTemperatures(
        temperatures: DeviceCapability[],
        temperatureSettings: TemperatureSettings | undefined,
    ): DeviceCapability[] {
        if (
            temperatureSettings &&
            temperatureSettings.validate &&
            temperatureSettings.validate_min !== undefined &&
            temperatureSettings.validate_max !== undefined
        ) {
            const dcs: DeviceCapability[] = [];
            ...
            if (temperatures.length !== dcs.length) {
                this.logger?.verbose(
                    `Validated temperatures: Got ${temperatures.length} temperatures, returned ${dcs.length} temperatures:`,
                    temperatures,
                    dcs,
                );
            }
    ...
    calculateTargetTemperatures(device: Device, zone: Zone): DeviceRequests {
        ...
        if (targetSettings.zone?.from_other) {
            ...
            if (otherThermostatsInZone && otherThermostatsInZone.length > 0) {
                ...
                const toTargetTemperature: DeviceCapability | undefined =
                    otherThermostatsInZone[0].getLocalCapabilityValue('target_temperature');
        ...
        const otherZoneIds = this.zonesObj
            .getZonesAsList(zone)
            .map(z => z.id)
            .filter(id => id !== zone.id && (!childrenZoneIds || !childrenZoneIds.includes(id)));

        const thermostats = this.devicesObj
            .getDevices()
            ?.filter(
                d =>
                    d.id !== device.id &&
                    ...
                    ((targetSettings.zone?.to_other && zone.id === d.zone && !d.isVThermo() && !d.isVHumidity()) ||
                        (targetSettings.sub_zones?.to_vthermo &&
                            !!childrenZoneIds &&
                            childrenZoneIds.includes(d.zone) &&
                            d.isVThermo()) ||
        ...
        if (thermostats) {
            for (const thermostat of thermostats) {
                ...
                const targetUpdateEnabled =
                    !thermostat.isVThermo() ||
        ...
        for (const thermostat of thermostats) {
            ...
            if (thTargetTemperature && thTargetTemperature.value && thTemperature && thTemperature.value) {
                ...
                if (newTargetTemperature) {
                    const dr = this.updateAndCreateDeviceRequestIfChanged(
                        thermostat,
                        'target_temperature',
