...
describe("findTemperature", function () {
    describe("No temp sensor in zone", function () {
        it("Check undefined", async function () {
            ...
            let tmp = await temperatureLib.findTemperature(device, 'no-such-zone', devices);
    ...
    describe("Check temp in zone", function () {
        it("Check temp 17.9", async function () {
            const device = devs.getDevice();
            let tmp = await temperatureLib.findTemperature(device, '9eb2975d-49ea-4033-8db0-105a3e982117', devices);
        ...
        it("Check temp 17.9 not changed", async function () {
            const device = devs.getDevice();
        ...
        it("Check 17.25 average", async function () {
            const device = devs.getDevice();
            let tmp = await temperatureLib.findTemperature(device, '1815c884-af06-4d53-a2c1-6f4c77e9eb4e', devices);
