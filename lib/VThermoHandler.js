...
module.exports = class VThermoHandler extends BaseHandler {
    ...
    async onCapability(event) {
        ...
        } else if (event.capId === 'alarm_motion') {
            return (await this._onAlarmMotion(event));
        }
    }
    ...
    async updateAllTargetTemperatures() {
        for (let device of this._driver.getDevices()) {
            const id = device.getId();
            const node = this._deviceHandler.getDevice(id);
            if (node && node.capabilitiesObj['target_temperature']) {
                const value = device.getCapabilityValue('target_temperature');
                await this._onTargetTemperature({ id, value });
            }
        }
    }
            ...
            if (node) {
                ...
                for (let thermoNode of thermos) {
                    ...
                    if (vThermoDevice) {
                        ...
                        targetUpdateEnabled = targetSettingsVThermo.target_update_enabled;
                    }

                    if (targetUpdateEnabled &&
                        thermoNode.capabilitiesObj['target_temperature'].value !== targetTemperature) {
                        thermoDevice.setCapabilityValue('target_temperature', targetTemperature)
                            ...
                            .catch(err => {
                                if (err.name && err.name === 'invalid_step') {
                                    targetTemperature = Math.round(2 * targetTemperature) / 2;
                                    thermoDevice.setCapabilityValue('target_temperature', targetTemperature)
                                        .then(() => this.log(`Updated: "${thermoNode.name}": target temperature set to ${targetTemperature} from ${node.name}`))
                                        .catch(err => this.log(`Update: "${thermoNode.name}": FAILED`, err));
                                } else {
                                    this.log(`Update: "${thermoNode.name}": FAILED`, err);
                                }
                            });
                    }
                }
            ...
            for (let vThermoNode of vThermosInZone) {
                ...
                if (vThermoDevice) {
                    ...
                    if (targetSettings.zone.from_other &&
                        vThermoNode.capabilitiesObj['target_temperature'].value !== event.value) {
    ...
    async _onAlarmMotion(event) {
        ...
        if (node) {
            ...
            for (let vThermoNode of vThermosInZone) {
                const vThermoDevice = this.getDevice(vThermoNode.id);
                if (vThermoDevice && vThermoDevice.getSetting('motion_alarm')) {
                    await this.switchDevice(vThermoDevice);
                }
            }
        }
    ...
    _calcTemperature(temperatures, tempSettings) {
        ...
        if (temperatures.length > 0) {
            ...
            } else if (tempSettings.calcMethod === constants.CALC_METHODS.NEWEST) {
                temperature = math.newest(temperatures);
            } else if (tempSettings.calcMethod === constants.CALC_METHODS.MANUAL) {
    ...
    _updateMeasureTemperature(deviceId, device, node, tempSettings, temperature) {
        ...
        if (curValue !== temperature && temperature !== undefined || temperature === null) {
    ...
    async updateTemperature(device, temperature) {
        if (!device) {
            this.log('updateTemperature: missing device');
            return;
        }

        const node = this._deviceHandler.getDevice(device.getId());
        if (!node) {
            this.log(`updateTemperature: missing node for ${device.getId()}`);
            return;
        }

        if (!node.capabilitiesObj['measure_temperature']) {
            this.log(`updateTemperature: missing measure_temperature capability`);
            return;
        }

        node.capabilitiesObj['measure_temperature'].value = temperature;
        device.log('updateTemperature: updated measure_temperature: ', temperature);
    }
    ...
    async switchDevice(device) {
        ...
        if (node.capabilitiesObj['onoff'].value !== true &&
            !settings.onoff_enabled) {
            device.setCapabilityValue('onoff', true).catch(err => this.log(err));
            node.capabilitiesObj['onoff'].value = true;
        }

        const contactAlarm = settings.contact_alarm && this._deviceHandler.hasContactAlarm(node.zone);
        const motionAlarm = settings.motion_alarm && this._deviceHandler.hasMotionAlarm(node.zone);
        const onoff = this._resolveOnOff(settings, node, targetTemp, temperature, contactAlarm, motionAlarm);
    ...
    _resolveOnOff(settings, node, targetTemp, temperature, contactAlarm, motionAlarm) {
